...
class ItemDataFetcher:
    ...
    def fetch_data(self):
        ...
        self.sells_dataframe = self.sells_dataframe.sort_values(by='ds').reset_index(drop=True)
        self.vending_dataframe = self.vending_dataframe.sort_values(by='ds').reset_index(drop=True)
    ...
    def _process_sells(self, sell_history):
        for sell_iterate in range(0, len(sell_history)):
            ...
            for sell_iterate_on_day in range(0, len(sell_history[sell_iterate]['y'])):
                ...
                self.sells_dataframe = self.sells_dataframe._append({'ds': fecha, 'y': precio_venta}, ignore_index=True)
        self.sells_dataframe['ds'] = pd.to_datetime(self.sells_dataframe['ds']).dt.tz_localize(None)  # Convertir a tz-naive
    ...
    def _process_vending(self, vend_history):
        for vending_iterate in range(0, len(vend_history)):
            ...
            for vending_iterate_on_day in range(0, len(vend_history[vending_iterate]['y'])):
                ...
                self.vending_dataframe = self.vending_dataframe._append({'ds': fecha, 'y': precio_venta_ofrecido}, ignore_index=True)
        self.vending_dataframe['ds'] = pd.to_datetime(self.vending_dataframe['ds']).dt.tz_localize(None)  # Convertir a tz-naive
    ...
    def plot_data(self):
        ...
        plt.plot(self.vending_dataframe['ds'], self.vending_dataframe['y'], label='Vending')
        plt.xlabel('Fecha')
        plt.ylabel('Precio')
